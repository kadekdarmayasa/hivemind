...
export default function Textarea(props: TextAreaProps) {
  ...
  return (
    <div className={`${outerClassNames.join(" ")}`}>
      ...
      <textarea name={name} id={id} onChange={onChange} onBlur={onBlur} onFocus={onFocus} className={`bg-[#F2F3FF] placeholder:text-brave-purple h-36 px-5 py-3 rounded-lg text-coarse-wool text-lg font-light border-2 outline-none resize-none ${hasError ? 'border-red-400' : 'border-none'} transition-all`} autoComplete="off" required placeholder={placeHolder}>{value}</textarea>
