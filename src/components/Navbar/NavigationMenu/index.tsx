...
export default function NavigationMenu({ href, name, containSubMenu, subMenu }: NavigationMenuProps): JSX.Element {
  ...
  if (containSubMenu) {
    return (
      <li tabIndex={0} className={`z-50 text-base font-regular relative text-brave-purple ml-10 nav-link ${dropdownState === 'open' || router.asPath.split('/')[1] === 'service' ? 'active' : ''}`} onBlur={() => dispatch(toggle('close'))} onFocus={() => dispatch(toggle('open'))} onMouseEnter={() => dispatch(toggle('open'))} onMouseLeave={() => dispatch(toggle('close'))}>
        ...
        <ul
          className={`font-light absolute z-50 w-80 bg-white rounded-lg shadow-black-md mt-4 px-4 py-5 right-2/4 translate-x-2/4 transition-all origin-top ${dropdownState === 'open' ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}
        >
